...
Const
  ...
  FCoef2 : array[0..30] of CTypes.cfloat =
  (
...
function chebyLP2(const f : CTypes.cfloat) : CTypes.cfloat;
Var
   n : Integer;
   y : CTypes.cfloat;
Begin
...
begin
     ...
     // Adjust to float (from samps[]) and copy data to FFT calculation buffer (glf1buffer[])
     fsum := 0.0;
     fave := 0;
     for i := 0 to jz-1 do fsum := fsum + samps[i];
     fave := fsum/jz;
     for i := 0 to jz-1 do glf1Buffer[i] := samps[i]-fave;
     fsum := 0.0;
     fave := 0.0;
     for i := 0 to jz-1 do
     Begin
          glf1Buffer[i] := 0.1 * glf1Buffer[i];
          fsum := fsum + glf1Buffer[i];
     End;
     fave := fsum/jz;
     for i := 0 to jz-1 do glf1Buffer[i] := glf1Buffer[i]-fave;
     ...
     // Apply lpf
     //function chebyLP(const f : CTypes.cfloat) : CTypes.cfloat;
     //for i := 0 to length(glf1Buffer)-1 do glf1Buffer[i] := chebyLP(glf1Buffer[i]);
     { TODO : This is rolling off too strongly on upper end seeing about 10dB cut within passband - adjust }
     for i := 0 to length(glf1Buffer)-1 do glf1Buffer[i] := chebyLP2(glf1Buffer[i]);
